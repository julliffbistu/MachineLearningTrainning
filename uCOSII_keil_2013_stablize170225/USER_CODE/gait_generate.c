...
#define GAIT_BUFFER_QUEUE_LENGTH	0x100
...
struct CurrentBodyState
{
	...
	struct RigidBody currentAnkle;
	Uint16 isLeft;
};


struct WalkStablizeHistogram
{
	INT8U rollHist[16];
	INT8U pitchHist[16];
	INT8U yawHist[16];
...
struct WalkStablizeHistogram walkStablizeHistogram;
...
struct Sensors angleDiff;
struct Sensors range;
	
Uint16 ResetStablizationProcedure()
{
	memset(&walkStablizeHistogram,0,sizeof(walkStablizeHistogram));
	walkStablizeHistogram.rollHist[8]=1;
	walkStablizeHistogram.pitchHist[8]=1;
	walkStablizeHistogram.yawHist[8]=1;
	walkStablizeHistogram.sampleCntr=0;
	return 1;
}

Uint16 FindMaxIndex(INT8U *phist, Uint16 size)
{
	int32 i=0;
	int32 maxIndex=0;
	INT8U max=0;
...
/*
Uint16 GetAngleDifferenceForStablization(struct Sensors *pdiff, struct Sensors *prange)
{	
	int32 maxCntr=0;
	maxCntr=FindMaxIndex(walkStablizeHistogram.rollHist,sizeof(walkStablizeHistogram.rollHist));
	if(maxCntr==0)
		pdiff->incline[0]=-357;
	else if(maxCntr==sizeof(walkStablizeHistogram.rollHist)-1)
		pdiff->incline[0]=357;
	else
		pdiff->incline[0]=(maxCntr-8)*45;
	prange->incline[0]=	357;

	maxCntr=FindMaxIndex(walkStablizeHistogram.pitchHist,sizeof(walkStablizeHistogram.pitchHist));
	if(maxCntr==0)
		pdiff->incline[1]=-357;
	else if(maxCntr==sizeof(walkStablizeHistogram.pitchHist)-1)
		pdiff->incline[1]=357;
	else
		pdiff->incline[1]=(maxCntr-8)*45;
	prange->incline[1]=	357;
...
{	
	...
	if(maxCntr==0)
		...
		pdiff->incline[0]=(maxCntr-8)*23;
	prange->incline[0]=	179;
	...
	if(maxCntr==0)
		...
		pdiff->incline[1]=(maxCntr-8)*23;
	prange->incline[1]=	179;

	prange->omega[0]=400;
	if(walkStablizeHistogram.omegaRoll>(int32)walkStablizeHistogram.sampleCntr*prange->omega[0])
		pdiff->omega[0]=prange->omega[0];
	else if(walkStablizeHistogram.omegaRoll<-(int32)walkStablizeHistogram.sampleCntr*prange->omega[0])
		pdiff->omega[0]=-prange->omega[0];
	else
		pdiff->omega[0]=(int16)(walkStablizeHistogram.omegaRoll/walkStablizeHistogram.sampleCntr);

	prange->omega[1]=400;
	if(walkStablizeHistogram.omegaPitch>(int32)walkStablizeHistogram.sampleCntr*prange->omega[1])
		pdiff->omega[1]=prange->omega[1];
	else if(walkStablizeHistogram.omegaPitch<-(int32)walkStablizeHistogram.sampleCntr*prange->omega[1])
		pdiff->omega[1]=-prange->omega[1];
	else
		pdiff->omega[1]=(int16)(walkStablizeHistogram.omegaPitch/walkStablizeHistogram.sampleCntr);
...
Uint16 GetAngleDifferenceForStablization(struct Sensors *pdiff, struct Sensors *prange)
{
	//the maximum incline angle is 20 deg
	prange->incline[0]=179;
	if(walkStablizeHistogram.angleRoll>(int32)walkStablizeHistogram.sampleCntr*prange->incline[0])
		pdiff->incline[0]=prange->incline[0];
	else if(walkStablizeHistogram.angleRoll<-(int32)walkStablizeHistogram.sampleCntr*prange->incline[0])
		pdiff->incline[0]=-prange->incline[0];
	else
		pdiff->incline[0]=(int16)(walkStablizeHistogram.angleRoll/walkStablizeHistogram.sampleCntr);

	prange->incline[1]=179;
	if(walkStablizeHistogram.anglePitch>(int32)walkStablizeHistogram.sampleCntr*prange->incline[1])
		pdiff->incline[1]=prange->incline[1];
	else if(walkStablizeHistogram.anglePitch<-(int32)walkStablizeHistogram.sampleCntr*prange->incline[1])
		pdiff->incline[1]=-prange->incline[1];
	else
		pdiff->incline[1]=(int16)(walkStablizeHistogram.anglePitch/walkStablizeHistogram.sampleCntr);

	//the maximum angular velocity of omega is 45 deg/s
	prange->omega[0]=804;
	if(walkStablizeHistogram.omegaRoll>(int32)walkStablizeHistogram.sampleCntr*prange->omega[0])
		pdiff->omega[0]=prange->omega[0];
	else if(walkStablizeHistogram.omegaRoll<-(int32)walkStablizeHistogram.sampleCntr*prange->omega[0])
...
#define MULTI_OMEGA_CONSTANT(a)	((a)/5)

Uint16 SingleStepControlWithStablization()
{
	//current supporting leg should be right and swinging leg should be left
	//struct Sensors angleDiff;
	//struct Sensors range;
	Uint16 frontBackPeriod=0;
	Uint16 leftRightPeriod=0;
	GetAngleDifferenceForStablization(&angleDiff,&range);
	
	//intialize realPeriod to 1.0
	gDC.realPeriod=512;

//eliminate any falling protection procedure
/*
	SingleStepControl();
	return 1;
	//no step moduling issued, only time tunning issued
	if(angleDiff.incline[0]>=range.incline[0])
	{
		//no modification should be issued falling protection is needed
		//the robot is falling to the right side
		SingleStepControl();
		return 1;
	}else if(angleDiff.incline[0]<=-range.incline[0])
	{
		//no modification should be issued falling protection is needed
		//the robot is falling to the left side
		SingleStepControl();
		return 2;
	}

	//no step moduling issued, only time tunning issued
	if(angleDiff.incline[1]>=range.incline[1])
	{
		//no modification should be issued falling protection is needed
		//the robot is falling to the front side
		SingleStepControl();
		return 3;
	}else if(angleDiff.incline[1]<=-range.incline[1])
	{
		//no modification should be issued falling protection is needed
		//the robot is falling to the back side
		SingleStepControl();
		return 4;
	}
*/
	...
	if(gDC.lastFoot==0)
	{
		//stepping right completion
		if(gDC.lastFootDir.yOffset>0)
		{
			//suppose the max delta distance is 50, distance between two foot is 100mm and effeicent is 1/2
			//in this condition when 30d is inclined then 1.25*period is generated
			//leftRightPeriod=512+SIN(angleDiff.incline[0])*100/50/512*512/2;
			leftRightPeriod=512+SIN(angleDiff.incline[0]+MULTI_OMEGA_CONSTANT(angleDiff.omega[0]));
			gDC.thisFootDir.yOffset=0;
			gDC.thisHipDir.yOffset=0;
		}else if(gDC.instEffect.yOffset>=0)	//stepping left start
		{
			//suppose 20mm stride is used to module COG
			gDC.thisFootDir.yOffset=(int16)((int32)gDC.instEffect.yOffset-((int32)angleDiff.incline[0]*30/range.incline[0]));
			gDC.thisFootDir.yOffset-=(int16)((int32)angleDiff.omega[0]*20/range.omega[0]);
			if(gDC.thisFootDir.yOffset<0)
				gDC.thisFootDir.yOffset=0;	
			//leftRightPeriod=512+SIN(angleDiff.incline[0])*(100+gDC.thisFootDir.yOffset)/50/512*512/2;
			leftRightPeriod=(int16)(512+((int32)SIN(angleDiff.incline[0]+MULTI_OMEGA_CONSTANT(angleDiff.omega[0])))*(100+gDC.thisFootDir.yOffset)/100);

			gDC.thisHipDir.yOffset=(int16)((
				((int32)gDC.thisFootDir.yOffset)*
				walkingConfig.ypercentage
				)>>PRECISION_BIT);
			gDC.footMesuredSpeed.yOffset=gDC.instEffect.yOffset;
		}else								//no side stepping issued
		{
			//suppose the max delta distance is 50, distance between two foot is 100mm and effeicent is 1/2
			//in this condition when 30d is inclined then 1.25*period is generated
			//leftRightPeriod=512+SIN(angleDiff.incline[0])*100/50/512*512/2;			
		...
		//turnning right completion
		if(gDC.lastFootDir.thetaOffset>0)
		{
			gDC.thisFootDir.thetaOffset=0;
			gDC.thisHipDir.thetaOffset=0;
		}else if(gDC.instEffect.thetaOffset>=0)	//turnning left start
		{
			gDC.thisFootDir.thetaOffset=gDC.instEffect.thetaOffset;
			//gDC.thisHipDir.thetaOffset=(gDC.thisFootDir.thetaOffset>>1);
			gDC.thisHipDir.thetaOffset=(int16)((
	...
	//suppose 30mm stride is used to module COG
	gDC.thisFootDir.xOffset=(int16)((int32)gDC.instEffect.xOffset+(int32)(angleDiff.incline[1])*30/range.incline[1]);
	gDC.thisFootDir.xOffset+=(int16)((int32)angleDiff.omega[1]*20/range.omega[1]);

	//constrain the footstep to [-80, 80] for kinematic constraints	
	if(gDC.thisFootDir.xOffset>80)
		gDC.thisFootDir.xOffset=80;
	else if(gDC.thisFootDir.xOffset<-80)
		gDC.thisFootDir.xOffset=-80;
		
	//get current foot x placement according to last foot x and inst
	if(gDC.lastFootDir.xOffset*gDC.thisFootDir.xOffset>0)
	{
		gDC.thisFootDir.xOffset=0;
		gDC.footMesuredSpeed.xOffset=0;
	}
	...
	{
		gDC.thisFootDir.xOffset=gDC.thisFootDir.xOffset;
		gDC.footMesuredSpeed.xOffset=gDC.instEffect.xOffset;
	}

	//frontBackPeriod=512+SIN(angleDiff.incline[1])*gDC.thisFootDir.xOffset/50/512*512;
	if(gDC.thisFootDir.xOffset>=0)
		frontBackPeriod=(int16)(512-(abs((int32)SIN(angleDiff.incline[1]+MULTI_OMEGA_CONSTANT(angleDiff.omega[1]))))*gDC.thisFootDir.xOffset/50);
	else
		frontBackPeriod=(int16)(512+(abs((int32)SIN(angleDiff.incline[1]+MULTI_OMEGA_CONSTANT(angleDiff.omega[1]))))*gDC.thisFootDir.xOffset/50);
...
//the walking freqency is dependant on forward direction rather than sidal direction
/*
	if(abs((int32)frontBackPeriod-512)>=abs((int32)leftRightPeriod-512)+32)
		gDC.realPeriod=frontBackPeriod;
	else if(abs((int32)leftRightPeriod-512)>=abs((int32)frontBackPeriod-512)+32)
		gDC.realPeriod=leftRightPeriod;
	else
		gDC.realPeriod=((leftRightPeriod+frontBackPeriod)>>1);		
*/

	//when the front back period moduling is not primary, while sidal freqency moduling is dominated we use sidal freqency
	//in other cases we use front back moduling
	if(abs((int32)frontBackPeriod-512)<50&&abs((int32)frontBackPeriod-512)+50<abs((int32)leftRightPeriod-512))
		gDC.realPeriod=leftRightPeriod;
	else
		gDC.realPeriod=frontBackPeriod;	
	//gDC.realPeriod=512;
...
Uint16 FillInStepParameterWithPushRecovery(int16 *pframe,Uint16 len)
{
	int32 temp=0;
	...
	*/		
		...
		temp+=((int32)(angleDiff.incline[1])*180/range.incline[1]);
		temp+=((int32)angleDiff.omega[1]*0/range.omega[1]);
...
Uint16 UpdateHipOffset()
{
	//return 0;
	//test the current feedback only
	hip.offset.x=gDC.thisFootDir.xOffset;
	hip.offset.y=gDC.thisFootDir.yOffset;
	//hip.offset.z=gDC.thisFootDir.thetaOffset;
	hip.offset.z=gDC.lastFoot;
	//end of conversion
	return 0;
}
...
Bool GenerateGaitStep(void)
{
	...
	case 1:
	{
		...
		tempHead.frameLength=1;
		tempHead.period=1;
		AddNewGaitWithPose(&tempHead,singleFrameCommand.frameBuffer,NULL,&hip,gDC.realPeriod);
		Close(fs);		
	}
	break;
	case 2:
	...
	case 3:
	{
		...
		//SingleStepControl();	
		SingleStepControlWithStablization();	
		OSMutexPost(plckUpdateCommand);
		...
		//and update to hip ankle and leg parameter for inverse kinematics
		//FillInStepParameter((int16 *)tempGaitFrame,step);
		FillInStepParameterWithPushRecovery((int16 *)tempGaitFrame,step);
		...
		tempHead.frameStep=mechZeroPoint.joints*2;
		gDC.realPeriod=((gDC.realPeriod*tempHead.period)>>9);

		if(gDC.realPeriod==tempHead.period)
		//if(1)
		{
			UpdateHipOffset();
			AddNewGaitWithPose(&tempHead,singleFrameCommand.frameBuffer,NULL,&hip,gDC.realPeriod);		
			for(i=1;i<tempHead.period;i++)
			{
				...
				UpdateHipOffset();
				AddGaitDataWithPose(tempHead.frameStep,tempGaitFrame,NULL,&hip,gDC.realPeriod);
		...
		}else
		{
			...
			Uint16 lastCntr=0;
			Uint16 readStep=0;
			Uint16 sampleGaitFrame[MAX_ROBOT_JOINTS*2];

			//add new gait with certain period
			memcpy(sampleGaitFrame,tempGaitFrame,sizeof(tempGaitFrame));
			lastCntr=tempHead.period;
			tempHead.period=gDC.realPeriod;
			UpdateHipOffset();
			AddNewGaitWithPose(&tempHead,singleFrameCommand.frameBuffer,NULL,&hip,gDC.realPeriod);
			tempHead.period=lastCntr;
			lastCntr=0;

			for(i=1;i<gDC.realPeriod;i++)
			{
				tempCntr=i*(tempHead.period-1)/(gDC.realPeriod-1);
				readStep=tempCntr-lastCntr;				
				while(readStep--)
					Read(fs,sampleGaitFrame,step);
				memcpy(tempGaitFrame,sampleGaitFrame,sizeof(tempGaitFrame));
				lastCntr=tempCntr;
				...
				for(j=0;j<mechZeroPoint.joints;j++)
				{
					...
					speed=((int32)singleFrameCommand.frameBuffer[j]-
							(int32)singleFrameCommand.frameBuffer[j+mechZeroPoint.joints])*
							SERVO_SPEED_CONSTANT;
					speed=speed*tempHead.period/gDC.realPeriod;
					speed=(speed<0?-speed:speed);
					speed=(speed<=SPEED_DEGRADE_CONST?1:speed-SPEED_DEGRADE_CONST);
					speed=(speed>1023?0:speed);
...
/*
Uint16 DeltaToHist(int16 delta, INT8U* phist)
{
	Uint16 index=0;
	if(delta>=357)
	{
		//index 15 is storing falling counter
		phist[15]+=1;
	}
	else if(delta>-357)
	{
		//index 8 is indicating zero offset from desired pose
		index=delta/45+8;
		phist[index]+=1;
	}else if(delta<=-357)
	{
		//index 0 is storing falling counter
		phist[0]+=1;
	}
	return 1;
}
...
void UpdateOmega(int16 roll, int16 pitch, int16 yaw)
{
	walkStablizeHistogram.sampleCntr+=1;
	walkStablizeHistogram.omegaRoll+=roll;
	walkStablizeHistogram.omegaPitch+=pitch;
	walkStablizeHistogram.omegaYaw+=yaw;
	return;
}

void UpdateRollAndPitch(struct Sensors *ps)
{
	walkStablizeHistogram.sampleCntr++;

	walkStablizeHistogram.omegaRoll+=ps->omega[0];
	walkStablizeHistogram.omegaPitch+=ps->omega[1];

	walkStablizeHistogram.angleRoll+=ps->incline[0];
	walkStablizeHistogram.anglePitch+=ps->incline[1];
}

Uint16 UpdateStablizationProcedure()
{
	struct Sensors currSen;
	GetSensorResult(&currSen);

	//change relative to mechanical zero point
	currSen.incline[0]-=9;
	currSen.incline[1]-=20;
	currSen.incline[2]-=0;

	//data comes from the calibration of sensor when robot is standing still
	currSen.incline[0]=-currSen.incline[0];
	currSen.incline[1]=currSen.incline[1];
	currSen.incline[2]=-currSen.incline[2];
	
	currSen.omega[0]=-currSen.omega[0];	
	currSen.omega[1]=currSen.omega[1];
	currSen.omega[2]=-currSen.omega[2];

	//because the histogram will greatly decrease the precesion of  angle detection thus give it up
	/*
	DeltaToHist(currSen.incline[0]-currentBodyState.currentHip.pose.alpha,walkStablizeHistogram.rollHist);
	DeltaToHist(currSen.incline[1]-currentBodyState.currentHip.pose.beta,walkStablizeHistogram.pitchHist);
	UpdateOmega(currSen.omega[0],currSen.omega[1],currSen.omega[2]);
	*/

	//the relative angle is taken into count with respect to the generated trajectory
	currSen.incline[0]-=currentBodyState.currentHip.pose.alpha;
	currSen.incline[1]-=currentBodyState.currentHip.pose.beta;
	UpdateRollAndPitch(&currSen);
	//DeltaToHist(currSen.incline[2]-currentBodyState.currentHip.theta,walkStablizeHistogram.yawHist);
	return 1;
}

int16 GetNextConfigurationWithPoseAndStablization(Uint16 *pbuf,Uint16 size)
{
	//process stablization procedure which should be synchronized with joint command sending
	Uint16 i=0;
	int16 ret=0;
	Uint16 *pdata=NULL;
	INT8U err=0;
	...
	{		
		...
		{	
			...
			currentBodyState.isLeft=gaitBufferQueue.q[gaitBufferQueue.f];
			gaitBufferQueue.f=(gaitBufferQueue.f+1)%GAIT_BUFFER_QUEUE_LENGTH;

			if(currentGaitHead.frameStep&0x8000)
			{
				pdata=(Uint16 *)&currentBodyState.currentAnkle;
			...
			{
				currentBodyState.currentAnkle.offset.x=INVALID_RIGIDBODY_VALUE;
				currentBodyState.currentAnkle.offset.y=INVALID_RIGIDBODY_VALUE;
				currentBodyState.currentAnkle.offset.z=INVALID_RIGIDBODY_VALUE;
				currentBodyState.currentAnkle.pose.alpha=INVALID_RIGIDBODY_VALUE;
				currentBodyState.currentAnkle.pose.beta=INVALID_RIGIDBODY_VALUE;
				currentBodyState.currentAnkle.pose.theta=INVALID_RIGIDBODY_VALUE;
			}

			if(currentGaitHead.frameStep&0x4000)
			{
				ResetStablizationProcedure();
				pdata=(Uint16 *)&currentBodyState.currentHip;
				for(i=0;i<sizeof(struct RigidBody)/sizeof(Uint16);i++)
				{
					pdata[i]=gaitBufferQueue.q[gaitBufferQueue.f];
					gaitBufferQueue.f=(gaitBufferQueue.f+1)%GAIT_BUFFER_QUEUE_LENGTH;
	...
	}else
	{
		...
		{			
			...
			if(currentGaitHead.frameStep&0x4000)
			{
				//update the stablization histogram
				//in order to avoid vibration of gyro due to footstep collistion delete the start and end frame 
				int32 percentage=0;
				percentage=(int32)gaitStepCntr*512/currentGaitHead.period;
				if(percentage>100&&percentage<410)
					UpdateStablizationProcedure();
